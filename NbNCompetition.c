...
#pragma config(Motor,  port9,           BallPicker,    tmotorVex393HighSpeed_MC29, openLoop, reversed)
...
#define TRIGGER_STATE_SHOOTING 1
#define TRIGGER_STATE_RELOADING 2
...
task usercontrol()
{
	...
	assignPresets();
	start_speed = presets[0][1];
	last_canopy_angle = presets[0][0];
	...
	while (true)
	{
		...
		if (vexRT[Btn8UXmtr2]) {
			set_canopy_position(presets[0][0]);
			FwVelocitySet( &flywheel, presets[0][1], start_drive );
			launcher_flywheel_running = 1;
		}
		else if (vexRT[Btn8RXmtr2]) {
			set_canopy_position(presets[1][0]);
			FwVelocitySet( &flywheel, presets[1][1], start_drive );
			launcher_flywheel_running = 1;
		}
		else if (vexRT[Btn8DXmtr2]) {
			set_canopy_position(presets[2][0]);
			FwVelocitySet( &flywheel, presets[2][1], start_drive );
			launcher_flywheel_running = 1;
		}
		...
		if (lcd_count % 40 == 0) { // update only 1 time per second (to prevent control lag)
